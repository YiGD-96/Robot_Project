...
def movesx_o(pos1, pos2, ry_offset=0, above_offset=90, velocity=400, acc=200, gripper=True):
...
def stack_objects(pickup_pos, stack_positions, base_z_bozung):
    ...
    for place_pos in stack_positions:
        if cnt == 0:
            ...
            up_pos = posx([next_pos[0], next_pos[1], next_pos[2] + 10 - z_bozung, next_pos[3], next_pos[4], next_pos[5]])
            next_pos = posx(place_pos)
...
def main():
    ...
    ref_pos = posx([367.320, 200.000, 60.000, 94.76, 130.00, 94.42])

    pickup_pos = ref_pos
    stack_positions_1 = stack(550, 80, -4, 3, 80)
    stack_objects(pickup_pos, stack_positions_1, base_z_bozung=1.65)

    release()
    pickup_pos = posx([ref_pos[0], ref_pos[1], ref_pos[2] -33, ref_pos[3], ref_pos[4], ref_pos[5]])
    movesx_o(stack_positions_1[-1], pickup_pos, acc=1000, gripper=False)
    stack_positions_2 = stack(550, 40, 90, 2, 80)
    stack_objects(pickup_pos, stack_positions_2, base_z_bozung=8.5)
    
    release()
    pickup_pos = posx([ref_pos[0], ref_pos[1], ref_pos[2] -55, ref_pos[3], ref_pos[4], ref_pos[5]])
    top_pos = posx([550.000, 0, 185.000, 94.76, 130.00, 94.42])
    movesx_o(stack_positions_2[-1], pickup_pos, above_offset=100, acc=1000, gripper=False)
